# Android Post-Merged-Java-Res Continuation

## Purpose
This document records CP-028 execution of only the first exact downstream continuation surface after successful `.\gradlew.bat :app:mergeOssDebugJavaResource --stacktrace`.

The checkpoint remained bounded to `.\gradlew.bat :app:mergeOssDebugJniLibFolders --stacktrace` and explicitly did not continue into native-lib merge, signing, packaging, install, or assemble work.
...
- no native-lib merge, signing, packaging, install, or assemble continuation
...
## Exact Prerequisite State Before Probe Entry
```text
...
android/fork/app/build/intermediates/merged_jni_libs/ossDebug/mergeOssDebugJniLibFolders => False
...
Only the existing bounded libcore prerequisite path was reused. Transient `android/fork/app/libs/libcore.aar` was reproduced before Gradle entry.
...
Exact direct shell transcript (full):
- `docs/android/evidence/cp028_mergeOssDebugJniLibFolders.clean.log`

Key exact output lines:
```text
> Task :app:mergeOssDebugJniLibFolders
...
The first exact meaningful outcome was success of `:app:mergeOssDebugJniLibFolders`.
...
Removed exactly per CP-028 expectations:
- `.tmp/cp028-gomobile-src`
- `.tmp/cp028-gopath`
...
## Outcome
CP-028 execution is complete and successful.

Confirmed:
- exact bounded probe command was executed once
- first exact meaningful outcome was success of `:app:mergeOssDebugJniLibFolders`
- expected success path now exists: `android/fork/app/build/intermediates/merged_jni_libs/ossDebug/mergeOssDebugJniLibFolders`
- out-of-scope native-lib/signing/packaging/install/assemble paths remained absent

Explicit separation preserved:
- continuation probing stopped at `mergeOssDebugJniLibFolders` and did not broaden into native-lib merge or later build surfaces

## Exact Next Surface
The next checkpoint should define only the smallest bounded continuation after successful `:app:mergeOssDebugJniLibFolders`, most likely:
