# Android Post-Javac Continuation

## Purpose
This document records the CP-020 execution that probed only the first exact downstream build surface after successful `.\gradlew.bat :app:compileOssDebugJavaWithJavac --stacktrace`.

The checkpoint stayed narrow:
- the cleared `libcore` metadata bridge was reused only as an existing prerequisite through the same disposable gomobile path already validated in CP-014 through CP-019
...
- no Gradle task broader than `.\gradlew.bat :app:bundleOssDebugClassesToCompileJar --stacktrace` was executed intentionally
...
- confirmed the downstream output state before the probe:
  - `compile_app_classes_jar` absent
  - runtime-jar output absent
  - dex output absent
  - transient `android/fork/app/libs/libcore.aar` absent
...
- entered the exact downstream compile-jar consumer surface with `.\gradlew.bat :app:bundleOssDebugClassesToCompileJar --stacktrace`
...
- verified that compile-jar output now exists and that runtime-jar and dex outputs still do not
- removed the disposable CP-020 tool workspaces, `android/fork/libcore/.build`, and transient copied `android/fork/app/libs/libcore.aar` after evidence capture

Not performed:
- no new metadata-bridge diagnosis or repair
- no new sing-box alignment or persistence work
- no edits under `android/fork/`
- no runtime-jar merge, dexing, packaging, install, assemble, or APK generation work
...
## Pre-Probe Artifact Boundary
Observed before regenerating the transient prerequisite:

```text
compile_app_classes_jar_exists=False
runtime_jar_exists=False
project_dex_archive_exists=False
libcore_aar_exists=False
```

## Disposable Tool Baseline Used
Temporary paths used during CP-020:
- source workspace: `C:\Users\grUm.IGOR\Documents\mobile-network-policy-lab\.tmp\cp020-gomobile-src`
- isolated GOPATH and patched binaries: `C:\Users\grUm.IGOR\Documents\mobile-network-policy-lab\.tmp\cp020-gopath`

Exact disposable rebuild commands:

```powershell
$env:GOPATH='C:\Users\grUm.IGOR\Documents\mobile-network-policy-lab\.tmp\cp020-gopath'
$env:GOTOOLCHAIN='auto'
Set-Location 'C:\Users\grUm.IGOR\Documents\mobile-network-policy-lab\.tmp\cp020-gomobile-src'
...
C:\Users\grUm.IGOR\Documents\mobile-network-policy-lab\.tmp\cp020-gopath\bin\gomobile-matsuri.exe: go1.24.0
...
The prerequisite artifact before compile-jar entry remained:
...
- the build completed through `aar: classes.jar`, `aar: jni/.../libgojni.so`, and final Java stub packaging without surfacing a new blocker
...
Length        : 37518765
LastWriteTime : 16.04.2026 11:03:05
```

## First Downstream Compile-Jar Probe
Why this is the correct next exact surface:
- CP-019 had already produced javac outputs
- the first absent downstream output family before CP-020 was `android/fork/app/build/intermediates/compile_app_classes_jar/ossDebug`
- local Gradle task metadata had already identified `bundleOssDebugClassesToCompileJar` as the matching variant-specific compile-jar task
- runtime-jar and dex outputs were still absent before entering this probe
...
> Task :app:bundleOssDebugClassesToCompileJar
...
```text
compile_app_classes_jar_exists=True
runtime_app_classes_jar_exists=False
project_dex_archive_exists=False

C:\Users\grUm.IGOR\Documents\mobile-network-policy-lab\android\fork\app\build\intermediates\compile_app_classes_jar\ossDebug\bundleOssDebugClassesToCompileJar\classes.jar
...
Interpretation:
- the compile-jar consumer surface is not the next blocker
- the first meaningful downstream outcome was success at `:app:bundleOssDebugClassesToCompileJar`
- the probe remained bounded and did not cross into runtime-jar merge or dexing

## Cleanup After Evidence Capture
Removed after documentation:
- `.tmp/cp020-gomobile-src`
- `.tmp/cp020-gopath`
...
```text
exists_after_cleanup=False 	C:\Users\grUm.IGOR\Documents\mobile-network-policy-lab\.tmp\cp020-gomobile-src
exists_after_cleanup=False 	C:\Users\grUm.IGOR\Documents\mobile-network-policy-lab\.tmp\cp020-gopath
...
## Outcome
CP-020 completed the requested bounded continuation probe.

It proved:
- the prerequisite artifact before this surface remains `android/fork/app/libs/libcore.aar`
- the exact downstream consumer command is `.\gradlew.bat :app:bundleOssDebugClassesToCompileJar --stacktrace`
- that bounded compile-jar surface succeeds under the persisted sing-box baseline
- no new blocker appears at that surface

It did not perform:
- runtime-jar merge continuation
- dex or packaging work
- broader Gradle repair
- Android feature work

## Exact Next Surface
The next exact smallest downstream surface is no longer compile-jar merge.

The next checkpoint should define only the first bounded continuation after successful `:app:bundleOssDebugClassesToCompileJar`, most likely the runtime-jar merge surface entered by `.\gradlew.bat :app:bundleOssDebugClassesToRuntimeJar --stacktrace`, without continuing into dexing, packaging, or `assemble*`.
