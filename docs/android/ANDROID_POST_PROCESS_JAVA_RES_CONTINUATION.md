# Android Post-Process-Java-Res Continuation

## Purpose
This document records the CP-027 execution that probed only the first exact downstream build surface after the observed successful `.\gradlew.bat :app:processOssDebugJavaRes --stacktrace` result.
...
- no Gradle task broader than `.\gradlew.bat :app:mergeOssDebugJavaResource --stacktrace` was intentionally executed
...
- verified all CP-027 prerequisite checks before probe entry
- recreated a disposable `gomobile-matsuri` source workspace at revision `17d6af34f6bd6d7e1e428e0c652c8b54a46bda4f`
- reapplied only the known metadata-preserving fallback behavior in that disposable clone
...
- re-verified prerequisites before Gradle entry
- entered the exact downstream surface with `.\gradlew.bat :app:mergeOssDebugJavaResource --stacktrace`
- stopped after the first exact meaningful outcome
- captured direct shell transcript, plus daemon-log evidence for continuity
- verified expected success artifact path and required out-of-scope absent paths
- removed disposable CP-027 tool workspaces, `android/fork/libcore/.build`, and transient `android/fork/app/libs/libcore.aar`

Not performed:
- no new metadata-bridge diagnosis or repair
- no new sing-box alignment or persistence work
- no edits under `android/fork/`
- no JNI/native merge, signing, packaging, install, or assemble work
...
Temporary paths:
- source workspace: `C:\Users\grUm.IGOR\Documents\mobile-network-policy-lab\.tmp\cp027-gomobile-src`
- isolated GOPATH and patched binaries: `C:\Users\grUm.IGOR\Documents\mobile-network-policy-lab\.tmp\cp027-gopath`

Exact disposable rebuild commands:

```powershell
$env:GOPATH='C:\Users\grUm.IGOR\Documents\mobile-network-policy-lab\.tmp\cp027-gopath'
$env:GOTOOLCHAIN='auto'
Set-Location 'C:\Users\grUm.IGOR\Documents\mobile-network-policy-lab\.tmp\cp027-gomobile-src'
...
## First Downstream `mergeOssDebugJavaResource` Probe
Exact entry command:
...
```text
> Task :app:processOssDebugJavaRes UP-TO-DATE
> Task :app:mergeOssDebugJavaResource

BUILD SUCCESSFUL in 3s
25 actionable tasks: 2 executed, 23 up-to-date
```

Fallback evidence source policy:
- not required for outcome determination in CP-027 because the direct shell transcript was complete
- continuity cross-check still captured from `C:\Users\grUm.IGOR\.gradle\daemon\8.10.2\daemon-13220.out.log`

Daemon cross-check lines:
...
```text
C:\Users\grUm.IGOR\Documents\mobile-network-policy-lab\android\fork\app\build\intermediates\merged_java_res\ossDebug\mergeOssDebugJavaResource\base.jar | len=531 | mtime=2026-04-22 12:19:34
```

Interpretation:
- first exact meaningful outcome was success of the bounded `mergeOssDebugJavaResource` continuation probe
...
```text
cp027_gomobile_src_exists_after_cleanup=False
cp027_gopath_exists_after_cleanup=False
libcore_build_exists_after_cleanup=False
libcore_aar_exists_after_cleanup=False
process_java_res_out_still_exists=True
merge_java_res_still_exists=True
merged_jni_libs_exists_after_cleanup=False
outputs_apk_exists_after_cleanup=False
```

## Outcome
CP-027 completed the requested bounded continuation probe.

It proved:
- exact probe command: `.\gradlew.bat :app:mergeOssDebugJavaResource --stacktrace`
- first meaningful outcome: success of the bounded `mergeOssDebugJavaResource` surface
...
It did not perform:
- JNI/native merge work
- signing, packaging, install, or assemble work
- Android feature work

## Exact Next Surface
The next exact smallest downstream surface is no longer `mergeOssDebugJavaResource`.

The next checkpoint should define only the first bounded continuation after successful `:app:mergeOssDebugJavaResource`, most likely `.\gradlew.bat :app:mergeOssDebugJniLibFolders --stacktrace`, without continuing into native-lib merge, signing, packaging, install, or `assemble*`.
