...
The next eligible work is to execute CP-041 only.
...
The next step is to execute CP-041 only in bounded scope.
...
- CP-041 execution artifact `docs/android/ANDROID_POST_RESUMED_TASK_SERVICE_VERIFICATION.md`
...
Execute CP-041 only: run one bounded service-state verification probe after prerequisite checks, capture first exact meaningful outcome, and stop without entering UI interaction, network activity, runtime debugging, transport logic, or per-app routing implementation.
...
Continuity will degrade if future work skips CP-041 execution and jumps directly into UI interaction, runtime debugging, network actions, or broad `assemble*` work.
...
If future work starts Android product implementation, per-app routing, transport logic, or broad build repair before CP-041 is executed in bounded scope, continuity and checkpoint discipline will degrade.
