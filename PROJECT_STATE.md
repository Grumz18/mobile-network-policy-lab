...
The next eligible work is to retry CP-041 execution only in bounded scope.
...
The next step is to retry CP-041 execution only in bounded scope.
...
Retry CP-041 execution only: keep the same bounded service-state scope, run prerequisites, execute one bounded probe once, capture first exact meaningful outcome, and stop without entering UI interaction, network activity, runtime debugging, transport logic, or per-app routing implementation.
...
Continuity will degrade if future work skips CP-041 retry execution and jumps directly into UI interaction, runtime debugging, network actions, or broad `assemble*` work.
...
If future work starts Android product implementation, per-app routing, transport logic, or broad build repair before CP-041 retry is completed in bounded scope, continuity and checkpoint discipline will degrade.
