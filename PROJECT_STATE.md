...
The next eligible work is to create CP-017 only.
...
## CURRENT_OBJECTIVE
Preserve all baselines created through CP-016 execution and the existing CP-014 metadata-bridge repair artifact.
...
The next step is to create CP-017 only and keep it limited to defining how that proven alignment should be established as a local dependency baseline without broadening into Gradle or app build continuation.
...
Create CP-017 only to define the smallest bounded checkpoint for establishing the proven `android/sing-box` alignment as the local dependency baseline, staying tool-local and out of Gradle or upstream `android/fork/` source.
...
If future work starts Android product implementation, per-app routing, transport logic, or broad build repair before CP-017 is defined and followed, continuity and checkpoint discipline will degrade.
