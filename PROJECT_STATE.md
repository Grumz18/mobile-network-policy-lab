...
The next eligible work is to execute CP-038 only.
...
The next step is to execute CP-038 only as the next bounded post-foreground-state continuation.
...
- CP-038 execution artifact `docs/android/ANDROID_POST_FOREGROUND_STATE_CONTINUATION.md` for the bounded post-foreground-state window-focus verification run
...
Execute CP-038 only to run the single bounded post-foreground-state window-focus verification probe and capture evidence, without entering UI interaction, runtime debugging, network actions, or feature work.
...
Continuity will degrade if future work skips CP-038 checkpoint execution and jumps directly into UI interaction, runtime debugging, network actions, or broad `assemble*` work.
...
If future work starts Android product implementation, per-app routing, transport logic, or broad build repair before CP-038 is executed and recorded under a bounded checkpoint, continuity and checkpoint discipline will degrade.
