...
The next eligible work is to create CP-029 only.
...
CP-028 execution then proved that `.\gradlew.bat :app:mergeOssDebugJniLibFolders --stacktrace` succeeds under the persisted sing-box baseline and creates `android/fork/app/build/intermediates/merged_jni_libs/ossDebug/mergeOssDebugJniLibFolders` while native-lib/signing/packaging/install/assemble outputs remain absent. The next step is to create CP-029 only so the first exact downstream continuation after successful `mergeOssDebugJniLibFolders` is defined without broadening into downstream build continuation.
...
Create CP-029 only to define the first exact bounded continuation surface after the observed successful `.\gradlew.bat :app:mergeOssDebugJniLibFolders --stacktrace` result, most likely `.\gradlew.bat :app:mergeOssDebugNativeLibs --stacktrace`, without broadening into signing, packaging, install, or assemble work.
...
Continuity will degrade if future work skips CP-029 definition for the next still-unexecuted downstream surface and jumps directly into native-lib merge, signing, packaging, or `assemble*` work.
...
If future work starts Android product implementation, per-app routing, transport logic, or broad build repair before CP-029 is defined and approved under a bounded checkpoint, continuity and checkpoint discipline will degrade.
