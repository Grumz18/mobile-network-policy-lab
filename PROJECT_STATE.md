...
The next eligible work is to restore exactly one online adb target (`device`, ABI `x86_64`) and retry CP-043 only in bounded scope.
...
Retry CP-043 only: restore exactly one online adb target with state `device` and ABI `x86_64`, then run the same bounded persistence-with-reporting validation flow without scope expansion.
