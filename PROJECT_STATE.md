...
The next eligible work is to execute CP-028 only.
...
The next step is to execute CP-028 only so the first exact `mergeOssDebugJniLibFolders` outcome can be captured without broadening into downstream build continuation.
...
Execute CP-028 only to probe the first exact bounded continuation surface after the observed successful `.\gradlew.bat :app:mergeOssDebugJavaResource --stacktrace` result with `.\gradlew.bat :app:mergeOssDebugJniLibFolders --stacktrace`, without broadening into native-lib merge, signing, packaging, install, or assemble work.
...
CP-028 now redefines the next exact downstream continuation surface as the bounded `.\gradlew.bat :app:mergeOssDebugJniLibFolders --stacktrace` probe aligned with the still-absent `android/fork/app/build/intermediates/merged_jni_libs/ossDebug/mergeOssDebugJniLibFolders` output family.
Continuity will degrade if future work skips the still-absent `mergeOssDebugJniLibFolders` surface and jumps directly into native-lib merge, signing, packaging, or `assemble*` work.
...
If future work starts Android product implementation, per-app routing, transport logic, or broad build repair before CP-028 is executed and closed under a bounded checkpoint, continuity and checkpoint discipline will degrade.
