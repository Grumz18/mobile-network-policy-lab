...
The next eligible work is to define the post-CP-013 repair checkpoint for the generated `libcore` module-metadata bridge only.
...
## CURRENT_OBJECTIVE
Preserve all baselines created through CP-013 execution and the new layout-blocker diagnosis artifact.
CP-013 proved that the current generated workspace intentionally stages only `gobind` sources and that the immediate failure comes from missing generated module metadata for bare import path `libcore`, not from a failed source copy into `.build/src`.
The next step is to define one exact repair checkpoint that tests only a tool-local generated `go.mod` metadata fallback for `libcore`, its local replacements, and the original `golang.org/x/mobile` baseline before retrying the native build path.
...
Define CP-014 to test only the isolated generated `go.mod` metadata fallback repair selected by CP-013, keeping the work tool-local and out of Gradle or upstream `android/fork/` source.
...
The next repair must stay on the selected metadata surface only. Broad source-copy changes, bind-invocation changes, or upstream Android fork edits would exceed the evidence gathered so far.
Version drift remains a risk: the original `libcore` graph still points to `golang.org/x/mobile v0.0.0-20231108233038-35478a0c49da`, but the fallback generated `go.mod` drifts to `v0.0.0-20260410095206-2cfb76559b7b` and forces Go toolchain auto-selection into `go1.25.x`.
...
If future work starts Android product implementation, per-app routing, transport logic, or broad build repair before the post-CP-013 repair checkpoint is defined and executed, continuity and checkpoint discipline will degrade.
