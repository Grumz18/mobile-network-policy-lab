...
The next eligible work is to execute CP-039 only.
...
The next step is to execute CP-039 only as the bounded post-CP-038 boundary-correction continuation.
...
- CP-039 execution artifact `docs/android/ANDROID_POST_FOCUS_BOUNDARY_CORRECTION.md` for bounded post-CP-038 focus-boundary correction
...
Execute CP-039 only to run the single bounded replacement focus/window probe and capture boundary-correction evidence without entering UI interaction, runtime debugging, network actions, or feature work.
...
Continuity will degrade if future work skips CP-039 checkpoint execution and jumps directly into UI interaction, runtime debugging, network actions, or broad `assemble*` work.
...
CP-039 now narrows the correction to a single replacement probe `dumpsys window | grep -m 1 -E 'mCurrentFocus|mFocusedApp'`; continuity will degrade if broader diagnostics or interaction flows start before this bounded correction is executed.
...
If future work starts Android product implementation, per-app routing, transport logic, or broad build repair before CP-039 is executed and recorded under a bounded checkpoint, continuity and checkpoint discipline will degrade.
